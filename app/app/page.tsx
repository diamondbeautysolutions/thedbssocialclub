...
const events = [
  { title: "The Social Edit", date: "Coming Soon", status: "RSVP opens soon" },
  { title: "DBS Social Club Mixer", date: "Coming Soon", status: "Members first" }
];

const news = [
  {
    title: "When your appointment ends, the conversation does not have to.",
    text: "The DBS Social Club continues the conversations, stories, culture, and connection that begin inside the salon."
  },
  {
    title: "Digital drops are coming soon.",
    text: "Classes, downloadable files, templates, and VIP-only resources will live inside the Products tab."
  }
];
  ...
  return (
    <div className="space-y-5 text-white">
      <header className="pt-2 text-center">
        <img src="/dbs-social-logo.svg" alt="The DBS Social Club" className="mx-auto h-20 w-auto invert" />
      </header>
      ...
      <section className="space-y-3">
        <div className="flex items-center justify-between"><h2 className="text-xl font-semibold">DBS Social News</h2><Newspaper className="h-5 w-5 text-white/50" /></div>
        {news.map((item) => (
          <article key={item.title} className="rounded-3xl border border-white/10 bg-white/10 p-5 shadow-xl">
      ...
      <section className="space-y-3">
        <div className="flex items-center justify-between"><h2 className="text-xl font-semibold">Events</h2><Link href="/app/events" className="text-sm text-white/50">View all</Link></div>
        {events.map((event) => (
